...
function main()
    # import *.pat files
    jldfile = "data/TEMP_advdiffAS_validation.jld"
    ...
    # time plots
    p2 = plot(title="Axisymmetric Advection-Diffusion Time-Accuracy",xlabel="||error||",ylabel="elapsed time (s)")
    ...
    scatter!(p3,harr,κarr,label="",ylabel=L"\kappa",
            title="Axisymmetric Advection-Diffusion condition number",markersize=MS,
            color=:blue)

    # export plots
    run(`mkdir -p figures`)
    plot(p1)
    savefig(p1,"figures/TEMPadvdiffAS_validation.png")
    plot(p2)
    savefig(p2,"figures/TEMPadvdiffAS_time_accuracy.png")
    plot(p3)
    savefig(p3,"figures/TEMPadvdiffAS_cond.png")
