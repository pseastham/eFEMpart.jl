...
function main()
  function computeNorms(N)
    ...
    #Fx(x,y) = -μ0*lapu(x,y) + dpdx(x,y)
    #Fy(x,y) = -μ0*lapv(x,y) + dpdy(x,y)
    Fx(x,y) = μ0*(12*x^2*y^2 + 2*x^4) + 3*x^2*y^3
    Fy(x,y) = -8*μ0*x*y*(x^2+y^2) + 3*x^3*y^2
  ...
  save("data/TEMP_stokes2D_const_validation.jld", "harr",harr,
                                          "VL1arr", L1arrV,
                                          "VL2arr", L2arrV,
                                          "VLInfarr",LInfarrV,
                                          "PL1arr", L1arrP,
