...
pyplot(size=(1200,800),linewidth=4,xscale=:log10,yscale=:log10,
       ...
       title="Stokes Equation (constant μ) Convergence")

function main()
    # import jld file
    jldfile = "data/TEMP_stokes2D_const_validation.jld"
    ...
    scatter!(p3,harr,κarr,label="",ylabel=L"\kappa",
            title="Stokes 2D (constant μ) condition number",markersize=10,
            color=:blue)

    # export plots
    run(`mkdir -p figures`)
    plot(p1)
    savefig(p1,"figures/TEMPstokes2D_const_validation.png")
    plot(p2)
    savefig(p2,"figures/TEMPstokes2D_const_time_accuracy.png")
    plot(p3)
    savefig(p3,"figures/TEMPstokes2D_const_cond.png")
