...
pyplot(size=(1200,800),linewidth=4,xscale=:log10,yscale=:log10,
       ...
       title="Stokes Equation (variable μ) Convergence")

function main()
    # import jld file
    jldfile = "data/TEMP_stokes2D_var_validation.jld"
    ...
    scatter!(p1,harr,PL1arr,label="L1",color=:red,label="")
    scatter!(p1,harr,PL2arr,label="L2",color=:blue,label="")
    scatter!(p1,harr,PLInfarr,label="Linf",color=:green,label="")
    ...
    scatter!(p1,harr,VL1arr,color=:red,label="L1",color=:red,label="")
    scatter!(p1,harr,VL2arr,color=:blue,label="L2",color=:blue,label="")
    scatter!(p1,harr,VLInfarr,color=:green,label="Linf",color=:green,label="") 
    plot!(p1,harr,0.04*harr.^4,color=:black,label="O(Δx^4)")   

    # time plots
    p2 = plot(title="Stokes (variable μ) Time-Accuracy",xlabel="||error||",ylabel="elapsed time (s)")
    ...
    scatter!(p3,harr,κarr,label="",ylabel=L"\kappa",
            title="Stokes 2D (variable μ) condition number",markersize=10,
            color=:blue)

    # export plots
    run(`mkdir -p figures`)
    plot(p1)
    savefig(p1,"figures/TEMPstokes2D_var_validation.png")
    plot(p2)
    savefig(p2,"figures/TEMPstokes2D_var_time_accuracy.png")
    plot(p3)
    savefig(p3,"figures/TEMPstokes2D_var_cond.png")
