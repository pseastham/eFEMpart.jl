...
function main()
    # import jld file
    jldfile = "data/TEMP_stokesAS_var_validation.jld"
    ...
    p3 = plot(harr,κarr,label="",linewidth=8,color=:blue)
        scatter!(p3,harr,κarr,label="",ylabel=L"\kappa",
            title="Stokes AS (variable μ) condition number",markersize=10,
            color=:blue)
    ...
    run(`mkdir -p figures`)
    plot(p1)
    savefig(p1,"figures/TEMPstokesAS_var_validation.png")
    plot(p2)
    savefig(p2,"figures/TEMPstokesAS_var_time_accuracy.png")
    plot(p3)
    savefig(p3,"figures/TEMPstokesAS_var_cond.png")
